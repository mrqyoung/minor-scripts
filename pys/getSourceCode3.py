...
def outputCode(flist):
    ...
    for f in flist:
        ...
        print(getPackageClass(codeLines), file = outputFile);
        print('-' * 64, file = outputFile);
...
def getPackageClass(listLines):
    package_class = '';
    for text in listLines:
        if (text.find('package') != -1):
            package_class = text[8:-2];
            continue;
        if (text.find('public') != -1):
            package_class = package_class + '/' + text.split(' ')[2];
